###--------------------------------------------------------------------------###
###---------------- Forecast code for non-conjugate VARs --------------------###
###--------------------------------------------------------------------------###
# 1.) Minnesota prior 
# 2.) Global-local shrinkage priors: 
#     Horseshoe prior 
#     Normal-gamma prior
#     Lasso prior (as a special case of the Normal-Gamma)
...
source(paste0(func.dir, "nconjVAR_func.R"))
...
#     Normal-gamma prior
#     Lasso prior (as a special case of the Normal-Gamma)
grid.nconj   <- c("nconjVAR-MIN", "nconjVAR-MINh", "nconjVAR-HS", "nconjVAR-NG", "nconjVAR-lasso")
grid.sv      <- "homo"  # c("homo", "sv")
...
run.sv      <- grid.full[run,"sv"]    # Model for conditional variance
...
mdl.setup <- list(
  "sv"            = run.sv,       # Stochastic volatility specification
  "prior"         = run.pr,       # Minnesota, HS, NG, and, LASSO
...
folder.name <- paste0(file.dir, run.mdl,"-", run.sv, "/")
...
if (!(file.check %in% folder.files)){
  ...
  tryCatch({
    est.mod <- nconjVAR.func(data.setup, mdl.setup, mcmc.setup)
    }, error = function(e) {
...
for(jrep in 1:nsave){
  X_th <- matrix(c(Y[N,],X[N,1:(M*(p-1))]),M*p,1) 
  ...
  O_th       <- matrix(0,Mp,Mp)
  SIG_th     <- est.mod$SIGt[jrep,N,,]
  L.inv_th   <- est.mod$L.inv[jrep,,]
  H_th       <- est.mod$Ht[jrep,N,]
  svpara_th  <- est.mod$svpara[jrep,,]
  ...
  for (hh in 1:fcst.hor){
    ...
    H_th    <- svpara_th["mu",] + svpara_th["phi",]*(H_th - svpara_th["mu",]) + svpara_th["sigma",]*rnorm(M)
    SIG_th  <- L.inv_th%*%diag(exp(H_th))%*%t(L.inv_th)
...
post.smry <- list(
  ...
  "tau_pr"    = apply(est.mod$tau_pr, c(2,3), mean),
  "lam_pr"    = apply(est.mod$lam_pr, c(2,3), mean))
